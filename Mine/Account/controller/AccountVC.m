...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        cell.textLabel.textColor = kWarmingRedColor;
        ...
        if (indexPath.section == 3 && indexPath.row == 2) {
            ...
            if (!cell) {
                cell = [[SystemSettingsCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"textCell"];
            }
            cell.detailTextLabel.text = @"经典白";
        }else if (indexPath.section == 4 && indexPath.row == 1) {
            cell = [tableView dequeueReusableCellWithIdentifier:@"skinCell"];
            if (!cell) {
                cell = [[SystemSettingsCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"skinCell"];
            }
            cell.rightLabel.hidden = NO;
            cell.rightLabel.text = @"  4.3.1  ";
        }else if (indexPath.section == 3 && indexPath.row == 3){
            cell = [tableView dequeueReusableCellWithIdentifier:@"nightCell"];
            if (!cell) {
                cell = [[SystemSettingsCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"nightCell"];
            }
            cell.swithState = YES;
            UISwitch *switchview = (UISwitch *)cell.accessoryView;
            [switchview addTarget:self action:@selector(swithChanged:) forControlEvents:UIControlEventValueChanged];
        }
...
- (void)swithChanged:(UISwitch *)swh{
